...
class StockEnvTrade(gym.Env):
    ...
    def __init__(self, df, day=0, turbulence_threshold=140,
                 initial=True, previous_state=[], model_name='', iteration='', file=None):
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            self.file.write("previous_total_asset: " + str(self.asset_memory[0]) + "\n")
            ...
            self.file.write("end_total_asset: " + str(end_total_asset) + "\n")
            self.file.write("total_reward: " + str(self.state[0] + sum(
                np.array(self.state[1:(STOCK_DIM + 1)]) * np.array(self.state[(STOCK_DIM + 1):(STOCK_DIM * 2 + 1)])) -
                                           self.asset_memory[0]) + "\n")
            self.file.write("total_cost: " + str(self.cost) + "\n")
            self.file.write("total trades: " + str(self.trades) + "\n")
            ...
            if df_total_value['daily_return'].std() != 0:
                ...
                sharpe = 0
            # print("Sharpe: ", sharpe)
            self.file.write("Sharpe: " + str(sharpe)  + "\n")
